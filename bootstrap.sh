...
export EMAIL=john@stantonnet.org
...
if [ ! -f "${VIM_PLUG_FILE}" ]; then
  ...
  git clone "https://github.com/prabirshrestha/asyncomplete-gocode.vim"
  popd
fi
...
if [ ! -d /root/code/dotfiles ]; then
  ...
  # if needed
  cd "/root/code"
  git clone --recursive https://github.com/ouchkernel/dotfiles.git

  cd "/root/code/dotfiles"
  git remote set-url origin git@github.com:ouchkernel/dotfiles.git
...
export OP_SESSION_my=$(op signin https://my.1password.com ${EMAIL} --output=raw)
